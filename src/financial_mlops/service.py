...
from .monitoring import latency_timer, log_event, new_request_id
...
def predict(request: PredictionRequest) -> PredictionResponse:
    """Validate features and run one model prediction."""
    ...
    with latency_timer() as elapsed:
        try:
            ...
            log_event("prediction_rejected", request_id=request_id, ticker=request.ticker, error=str(exc))
            raise HTTPException(status_code=422, detail=str(exc)) from exc
        except Exception as exc:  # defensive guard for service boundary
            log_event("prediction_failed", request_id=request_id, ticker=request.ticker, error=str(exc))
